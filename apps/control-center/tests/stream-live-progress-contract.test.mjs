...
test("live process rows render from codexActivity started entries", async () => {
  const mod = await source("public/modules/run-live-activity.js");
  assertIncludes(mod, "function codexLiveActivities(runId)");
  assertIncludes(mod, "function liveProcessRowsMarkup(run)");
  // 只接 command/file；reasoning 归呼吸行「正在思考」，一活不两显
  assertIncludes(mod, 'entry.progress.kind === "command" || entry.progress.kind === "file"');
...
test("breathing line stops duplicating command/file activity", async () => {
  const mod = await source("public/modules/run-live-activity.js");
  assertIncludes(mod, "const latestActivity = codexLiveActivities(run.id).at(-1);");
  assertIncludes(mod, 'const activity = latestActivity?.progress.kind === "reasoning" ? "正在思考" : "";');
...
test("turn progress bar: step counter + per-interaction file stats", async () => {
  const mod = await source("public/modules/run-live-activity.js");
  assertIncludes(mod, "function turnProgressMarkup(run)");
  assertIncludes(mod, "function trackTurnFileStats(event)");
  // 步进口径与顶栏 meta 同源
  assertIncludes(mod, "run.maxStepsPerInteraction ?? run.maxRounds");
  // 累加器重置/清账：新交互（user.message）与 run 收尾
  assertIncludes(mod, 'if (event.type === "user.message" || /^run\\.(completed|failed|cancelled)$/.test(event.type)) {');
  // 只收 completed 的 file progress，数行复用 diffLineStats（不造第二份）
  assertIncludes(mod, 'if (progress?.kind !== "file" || !Array.isArray(progress.changes)) return false;');
  assertIncludes(mod, "const lines = diffLineStats(change.diff);");
  assertIncludes(mod, 'data-stream-key="tail:progress"');
  // 无步进且无文件段时整条不渲染
  assertIncludes(mod, 'if (!stepText && !fileCount) return "";');
});
...
test("context compaction is visible in the session stream and live turn", async () => {
  const [app, mod] = await Promise.all([source("public/app.js"), source("public/modules/run-live-activity.js")]);
  assertIncludes(mod, '"run.context_compaction_started"');
  assertIncludes(mod, '"run.context_compaction_completed"');
  assertIncludes(mod, '"run.context_compaction_failed"');
  assertIncludes(mod, "正在压缩上下文，请勿中断");
  assertIncludes(mod, "function trackContextCompaction(event)");
