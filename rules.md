...
**读取纪律（2026-09-04 新增，因实测故障加入）**：追加式账本会无限增长——`decisions.md` 已达
**370KB / 4800+ 行（约 90-100K token）**，`handoff/` 156 个文件 2.5M。**"开工前先读 decisions.md"
这条老习惯现在会直接撑爆 subagent**：同日织两次委派 `Prompt is too long` / autocompact thrashing、
烛报 Codex 双通道超时，根因同一。对照实证：同一评审任务，任务卡写明"不要读治理上下文"后
300s 内完成，此前 400s 超时（`D-2026-09-04-003`）。

1. **主驾**：查历史决策用 `grep -n "^### D-" decisions.md | tail -30` 定位后按行段读，**不整读**。
2. **派 subagent 前先问「这个任务真的需要治理上下文吗」**——单文件评审 / 定点调研 / 聚焦实现都不需要，
   **在任务卡里显式写明不要读**。这是零风险、立即生效的止血措施。
3. 大范围调研拆成 ≤3 个单点小任务，不指望一个 agent 多批次检索。
